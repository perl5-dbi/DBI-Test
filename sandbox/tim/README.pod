...
Consider how we'll do thread tests. Might be another top-level variant that
adds "use threads;" at the start of the generated .t file. That'll run all test
scripts with "threads enabled" but no actual extra threads. Specific thread
test files could skip_all if run with out threads loaded or run DBI thread
tests if it is.

=head2 subtest issues with threads

See https://github.com/schwern/test-more/issues/388
and https://github.com/schwern/test-more/issues/263

Should we continue to use subtests when calling test methods?
Is there a subset of subtest functionality that's safe to use?
If not can we emulate subtest sufficiently?
